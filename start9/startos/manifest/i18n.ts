...
export const long = {
  ...
  es_ES:
    'Broadcast Pool se sitúa entre tu billetera (Sparrow o Liana) y el índice Electrum de tu nodo. Intercepta las difusiones de transacciones y retiene las transacciones firmadas en una mempool virtual hasta que se cumple el criterio elegido (inmediato, fecha/hora programada, precio fiat o altura de bloque). Se conecta automáticamente a Bitcoin Core y Fulcrum en StartOS y expone una interfaz Electrum TCP y un panel web. La red (mainnet, testnet4 o signet) se detecta automáticamente desde Bitcoin Core.',
}

export const alertInstall = {
  en_US:
    'Broadcast Pool requires Bitcoin Core (pruning disabled) and Fulcrum. A transaction shown as pending is retained in the virtual mempool and is NOT yet broadcast to the network until its trigger fires.',
  es_ES:
    'Broadcast Pool requiere Bitcoin Core (sin poda) y Fulcrum. Una transacción marcada como pendiente se retiene en la mempool virtual y NO se difunde a la red hasta que se dispara su criterio.',
}
...
export const fulcrumDescription = {
  en_US:
    'Provides the Electrum index backend (Fulcrum) for address and history lookups',
  es_ES:
    'Proporciona el backend de índice Electrum (Fulcrum) para direcciones e historial',
}
